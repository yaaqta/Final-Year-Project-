...
# --- CẤU HÌNH EMAIL ---
SENDER_EMAIL = "nguyentienphuonghaui@gmail.com"
SENDER_PASSWORD = "ilkdzvvfzypkhsar"
...
def send_smart_locker_email(receiver, subject, body):
    ...
    try:
        ...
        server.quit()
        print(f"[Email] Đã gửi thư thành công tới {receiver} - {subject}")
    except Exception as e:
        print(f"[Email] Lỗi khi gửi email tới {receiver}: {e}")
...
# ── Config ───────────────────────────────────────────────────
BASE_DIR = os.path.dirname(os.path.abspath(__file__))              # .../automatic_lockers/docs
PROJECT_DIR = os.path.abspath(os.path.join(BASE_DIR, '..'))        # .../automatic_lockers

TEMPLATE_DIR = os.path.join(PROJECT_DIR, 'templates')
DATA_DIR = os.path.join(PROJECT_DIR, 'data')
MODEL_PATH = os.path.join(PROJECT_DIR, 'yolov12n-face.pt')
...
def init_db():
    ...
    for col in ['gmail']:
        try:
            c.execute(f"ALTER TABLE lockers ADD COLUMN {col} TEXT")
        except Exception:
            pass
    # -----------------------------------------------------
    ...
    # -------- ĐOẠN NÀY SẼ FIX LỖI "no such column: gmail" --------
    try:
...
def log_access(username, action, confidence=None, gmail=None):
    conn = get_conn()
    conn.execute(
        "INSERT INTO access_logs (username, action, timestamp, confidence, gmail) VALUES (?,?,?,?,?)",
        (username, action, datetime.datetime.now().isoformat(), confidence, gmail)
    )
    conn.commit()
    conn.close()
...
def check_overdue_lockers():
    """Luồng chạy ngầm để kiểm tra tủ qua ngày lúc 08:00 sáng mỗi ngày"""
    while True:
        now = datetime.datetime.now()
        # Chạy kiểm tra vào lúc 08:00 sáng mỗi ngày
        if now.hour == 8 and now.minute == 0:
            print("[System] Bắt đầu quét các tủ để qua đêm...")
            conn = get_conn()
            c = conn.cursor()
            ...
            for locker in overdue_lockers:
                if locker['gmail']:
                    subject = f"Thông báo: Đồ ở Tủ {locker['locker_id']} đã qua ngày"
                    body = f"""
                    <h3>Chào {locker['username']},</h3>
                    <p>Hệ thống ghi nhận bạn đã gửi đồ tại <b>Tủ {locker['locker_id']}</b> từ lúc {locker['checkin_time']}.</p>
                    <p>Hiện tại đồ của bạn đã được lưu trữ qua ngày mới. Theo quy định, hệ thống sẽ <b>tính thêm phí lưu trữ qua đêm</b>.</p>
                    <p>Vui lòng đến nhận đồ sớm nhất có thể.</p>
                    <br><p><i>Hệ thống Smart Locker</i></p>
                    """
                    send_smart_locker_email(locker['gmail'], subject, body)
                    
            conn.close()
            time.sleep(61) # Ngủ 61 giây để không lặp lại trong 1 phút
            
        time.sleep(30)

# Bật luồng chạy ngầm ngay khi khởi động app
    ...
    try:
        if any(k.startswith('check_') or k.startswith('switch_') for k in data):
            ...
            for lid, vals in updates.items():
                ...
                # KIỂM TRA QUÊN ĐỒ VÀ GỬI EMAIL
                if has_items == 'yes':
                    locker_info = conn.execute("SELECT status, door_status FROM lockers WHERE locker_id=?", (lid,)).fetchone()
                    
                    # Chỉ báo quên đồ khi trạng thái tủ 'available' VÀ cửa đã đóng 'closed' (chống nhiễu cảm biến)
                    if locker_info and locker_info['status'] == 'available' and locker_info['door_status'] == 'closed':
                        ...
                        if last_log and last_log['gmail']:
                            ...
                            # Nếu vừa check-out trong vòng 1 phút
                            if seconds_since_checkout < 60:
                                ...
                                if cache_key not in app.sent_forgot_warnings:
                                    subject = f"Cảnh báo: Bạn đã để quên đồ ở Tủ {lid}"
                                    body = f"""
                                    <h3>Chào {last_log['username']},</h3>
                                    <p>Hệ thống ghi nhận bạn vừa lấy đồ tại <b>Tủ {lid}</b>.</p>
                                    <p>Tuy nhiên, cảm biến phát hiện <b>vẫn còn đồ vật bên trong tủ</b> sau khi cửa đã đóng lại. Bạn vui lòng quay lại kiểm tra để không bỏ quên tài sản nhé!</p>
                                    <br><p><i>Hệ thống Smart Locker</i></p>
                                    """
...
def register_locker():
    try:
        ...
        log_access(username, 'checkin', 1.0, gmail)
...
def face_checkout():
    try:
        ...
        log_access(owner, 'checkout', conf, row['gmail'])
        open_then_auto_close(locker_id)
...
def delete_logs():
    ...
    try:
        ...
        date = data.get('date')  # Format: "2026-04-09"
        if not date:
            return jsonify({'error': 'Thiếu ngày cần xóa'}), 400
            
        conn.execute("DELETE FROM access_logs WHERE date(timestamp) = ?", (date,))
        conn.commit()
        return jsonify({'message': f'Đã xóa lịch sử ngày {date}'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
    finally:
        conn.close()
